...
def generate_image(image_name,number,brightness):
  ...
  cv2.putText(image,number, (width//3-width//30,height//2), 0, 4, yuv_to_rgb(brightness_of_text,0,0),2)
  ## save image
...
brightness=128
## calculate step
step=brightness//int(sys.argv[3])

## loop to generate images
for i in range(int(sys.argv[3])):
  number=randrange(100,999)
  generate_image(str(i+1)+"_"+str(number)+".jpg",str(number),brightness)
